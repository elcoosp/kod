...
/// Error-path tests for `McpHost::ensure_started`. The happy path
/// (spawning a real server) needs a subprocess and a real MCP
/// handshake; these tests cover the two refusals the function
/// produces *before* any I/O, which are what a caller relies on to
/// distinguish "server not configured" from "server disabled".
#[cfg(test)]
mod coverage_mcp_errors {
    use super::*;

    fn host_with(entries: &[(&str, kod_config::McpServerConfig)]) -> McpHost {
        let mut cfg = kod_config::McpConfig::default();
        for (name, spec) in entries {
            cfg.servers.insert((*name).into(), spec.clone());
        }
        McpHost::new(cfg, std::env::temp_dir())
    }

    fn disabled(name: &str) -> (&str, kod_config::McpServerConfig) {
        (
            name,
            kod_config::McpServerConfig {
                enabled: false,
                command: "true".into(),
                ..Default::default()
            },
    ...
    fn commandless(name: &str) -> (&str, kod_config::McpServerConfig) {
        (
            name,
            kod_config::McpServerConfig {
                enabled: true,
    ...
    fn spawnable(name: &str) -> (&str, kod_config::McpServerConfig) {
        (
            name,
            kod_config::McpServerConfig {
                enabled: true,
    ...
    #[tokio::test]
    async fn ensure_started_error_is_invalid_state() {
        // The caller (a tool wrapper) formats the error for the model;
        // an `Internal` would be logged as a bug rather than surfaced
    ...
    #[test]
    fn server_names_of_an_empty_config_is_empty() {
        let host = host_with(&[]);
        assert!(host.server_names().is_empty());
    }
