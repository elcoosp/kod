...
| Shell-output minimizer (§5) | `kod-minimize/` | new crate: command classifier, 6 TOML defs (git-status, git-log, cargo-check, cargo-check-json, cargo-test, pytest), pipeline stages plus a native Rust filter path (`cargo-json`, `pytest-json`) for structured output; `execute_command` runs captured stdout through it and offloads the raw capture as an `artifact://` | 54 unit tests in the crate; 5 integration tests in `kod-tools/tests/minimizer_integration.rs` (real git repo, minimize + offload + round-trip) |
...
| Speculative read execution (§10) | `kod-core/src/speculation.rs` + `kod-core/src/engine/mod.rs` | streaming loop recognizes a `read_file` call when its `path` argument completes mid-stream (`extract_path_from_partial`), spawns the read, joins at round end; `run_tool_calls_with_speculations` validates each read (TOCTOU digest) and attaches the bytes to `ToolContext.prefetched_read`; `ReadFileTool` uses them without re-reading | 11 primitive tests + 8 partial-JSON tests; end-to-end wiring exercised by the streaming loop's existing tests |
| Snapcompact rasterization (§4.5) | `kod-core/src/snapcompact.rs` + `SnapcompactMethod` | dependency-free text→PNG: embedded 8×8 font, packed monochrome canvas, minimal PNG encoder (stored-mode deflate), hand-rolled base64. Method gates on provider `vision` + the design's `SAVINGS_MARGIN`, rasterizes the older half, returns an `Image` plan; the engine applies it and the Anthropic wire emits the image block | 11 rasterizer tests (PNG structure, glyphs, base64 vectors, CRC-32); dispatcher stub-test updated |## Deferred to a future slice, not-yet-built
| `xd://` lazy tool mounting (§6) | `kod-tools/src/xd_handler.rs` + engine demotion | `ToolDefinition.load_mode` (Essential/Discoverable); the `xd://` scheme lists, describes, and executes tools; `build_grounded_request` filters Discoverable tools out of the tools array. `batch` is the first Discoverable tool | 7 handler tests |
| WorkPool (§11.1) | `kod-swarm/src/work_pool.rs` | keep-alive worker slots consuming batches; dispatch policy = least-loaded idle by context ratio → spawn under `max_agents` → queue onto a running slot. Per-batch output schema (one required key per item id) and a yield-contract check; a slot that breaks it is tombstoned. `DispatchPolicy::FreshAgents` for one-slot-per-item | 20 tests |
| Park/revive registry (§11.2) | `kod-swarm/src/agent_registry.rs` | four lifecycle states (Idle/Active/Parked/Dead); park and revive keyed on a per-ref generation, so a stale finalizer cannot clobber a newer session. Depth-of walks the parent chain; park candidates are TTL-based. Cold revive across restarts is a documented follow-up | 19 tests |
| IrcBus (§11.3) | `kod-swarm/src/irc_bus.rs` | delivery receipts (Injected/Woken/Buffered/UnknownTarget/DroppedOldest), a bounded mailbox (`MAILBOX_CAP = 100` with oldest-drop), and `send_await` (correlation-id reply with fast-fail on a dead target and timeout cleanup) | 18 tests |
| YieldQueue (§11.5) | `kod-swarm/src/yield_queue.rs` | per-kind queue of thunks evaluated at drain time (staleness check sees drain-time state, not register-time); Streaming vs Idle flush modes; `skip_idle_flush` entries stay queued through an idle drain; thunks run outside the lock | 16 tests |
